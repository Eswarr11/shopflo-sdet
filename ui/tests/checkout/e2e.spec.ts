...
test.describe('Checkout E2E', { tag: '@regression' }, () => {
  test(
    'Verify Successful End-to-End Checkout for Standard User',
    { tag: '@smoke' },
  ...
  test('Verify Removed Inventory Items Do Not Appear in Checkout Flow', async ({
    page,
    poManager,
  }) => {
    ...
    await allure.step(
      'Remove two products from inventory and verify only one remains in cart state',
      async () => {
        const inventory = poManager.getInventoryPage();
  ...
  test('Verify User Can Add More Products and Complete Checkout After Reaching Order Overview', async ({
    page,
    poManager,
  }) => {
    await allure.feature('Checkout');
    ...
    await allure.step(
      'Verify updated order overview reflects all cart products and correct totals',
      async () => {
